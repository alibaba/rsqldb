...
public abstract class AbstractSelectParser extends AbstractSelectNodeParser<SqlSelect> {
    ...
    public IParseResult parse(SelectSqlBuilder selectSQLBuilder, SqlSelect sqlSelect) {
        ...
        //
        boolean isNeedWhereToCondition = parseFrom(selectSQLBuilder, sqlSelect.getFrom(), null);
        //
        parseWhere(selectSQLBuilder, sqlSelect, isNeedWhereToCondition);
        //
        parseGroup(selectSQLBuilder, sqlSelect);
        //
        //TODO parseHaving(); parseOn();
        //
        parseSelect(selectSQLBuilder, sqlSelect);
        //
        ...
        for (SqlNode sqlNode : sqlNodes.getList()) {
            ...
            if (builder.getOverWindowBuilder() != null) {
                ...
                if (CollectionUtil.isNotEmpty(varScripts)) {
                    sqlBuilder.putSelectField(varName, createScriptResult(builder));
                } else {
     ...
     * @param from
     */
    protected boolean parseFrom(SelectSqlBuilder rootSelectSQLBuilder, SqlNode from, String aliasName) {
...
////            String tableName = tableNames.iterator().next();
        ...
        } else if (SqlBasicCall.class.isInstance(from) && ((SqlBasicCall) from).getOperator().getName().toLowerCase().equals("values")) {
            ...
            parseRows(aliasName, from, rootSelectSQLBuilder, fieldNames, ((SqlBasicCall) from).getOperandList());
    ...
    protected void parseRows(String tableName, SqlNode rowSqlNode, SelectSqlBuilder selectSQLBuilder, List<String> fieldNames, List<SqlNode> rowList) {
        ...
        for (SqlNode rowNode : rowList) {
            if (!SqlBasicCall.class.isInstance(rowNode)) {
