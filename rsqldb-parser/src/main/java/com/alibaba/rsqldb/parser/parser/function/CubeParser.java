...
public class CubeParser extends AbstractSelectNodeParser<SqlBasicCall> {
    @Override public IParseResult parse(SelectSqlBuilder builder, SqlBasicCall sqlBasicCall) {
        ...
        if (fieldNames.size() > 0) {
            List<String> tmp = new ArrayList<>(fieldNames);
            List<String> rollupFieldNames = new ArrayList<>();
            while (tmp.size() > 0) {
                rollupFieldNames.add(MapKeyUtil.createKey(tmp));
                if (tmp.size() > 0) {
            ...
            List<String> cubeFieldNames = new ArrayList<>();
            tmp = new ArrayList<>(fieldNames);
            tmp.remove(0);
            while (tmp.size() > 0) {
                cubeFieldNames.add(MapKeyUtil.createKey(tmp));
                tmp.remove(0);
            }

            builder.getWindowBuilder().setGroupSetFieldNames(cubeFieldNames);
        }
        return null;
    }
    ...
    @Override public boolean support(Object sqlNode) {
        if (sqlNode instanceof SqlBasicCall) {
            ...
            if (sqlBasicCall.getOperator().getName().toUpperCase().equals("CUBE")) {
                return true;
            }
        }
        return false;
